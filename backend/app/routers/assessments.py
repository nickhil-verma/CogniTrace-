...
async def assess_audio_file(file: UploadFile = File(...)):
    """
    Ingests patient speech audio (WAV/WebM/MP3), extracts acoustic and linguistic digital biomarkers,
    ...
    try:
        acoustic = acoustic_extractor.extract_features(audio_bytes)
        linguistic = linguistic_extractor.extract_features(audio_bytes)
...
# ------------------------------------------------------------------
# Frontend Contract Endpoints (Direct Web & Flutter Compatibility)
# ------------------------------------------------------------------
...
async def submit_audio_task_turn(
    ...
    """
    Web App & Mobile App endpoint for patient voice interaction turns.
    """
    audio_bytes = b""
    if file:
        audio_bytes = await file.read()
    ...
    if audio_bytes:
        acoustic = acoustic_extractor.extract_features(audio_bytes)
        linguistic = linguistic_extractor.extract_features(audio_bytes, fallback_text=text)
    else:
        # Text-only or prompt submission fallback
    ...
    alert = report.risk_tier in ["MCI", "HIGH_RISK"]
    ai_response = (
